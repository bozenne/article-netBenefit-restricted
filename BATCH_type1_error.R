...
## * Prepare export
path <- "."
path.res <- file.path(path,"Results","type1_error")
...
path.output <- file.path(path,"output","type1_error")
...
for(iSim in 1:n.sim){  ## iSim <- 1
  cat(iSim,": ")
  for (iGrid in 1:NROW(grid)){ ## iGrid <- 12
    cat("*")
  ...
  saveRDS(res, file = file.path(path.res,paste0("type1_error",iter_sim,"(tempo).rds")))
  cat("\n")
}

## * Export
saveRDS(res, file = file.path(path.res,paste0("type1_error",iter_sim,".rds")))
